...
const useAuth = () => {
	const openNaverLogin = async () => {
		...
		const naver_id_login =
			new window.naver_id_login(process.env.REACT_APP_NAVER_CLIENT_ID, process.env.REACT_APP_NAVER_REDIRECT_URI) || null
		const state = naver_id_login.getUniqState()
		naver_id_login.setButton('white', 0, 40)
		naver_id_login.setDomain(process.env.REACT_APP_BASE_URL)
		naver_id_login.setState(state)
		naver_id_login.response_type = 'code'
		window.location.assign(
			`https://nid.naver.com/oauth2.0/authorize?response_type=code&client_id=CgJJKyt5o_NoOPtl51jX&redirect_uri=http%3A%2F%2Flocalhost%3A3000%2Foauth%3Ftype%3Dnaver&state=${state}`
		)
	}
