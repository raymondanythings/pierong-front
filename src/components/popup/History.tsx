...
const History = () => {
	const { userId } = store((state) => ({ userId: state.userId }))
	const { data: { data: owner = [] } = {}, refetch } = useQuery(['room', 'user', 'owner', userId], UserApi.getOwnerDetail, {
		cacheTime: Infinity,
		staleTime: 1000 * 60,
		retry: false,
	...
	return (
		<div className="grow flex flex-col rounded-b-2xl">
			<nav className="flex w-full">
				{locks.map((lock) => (
					<div
						key={lock}
						className={`grow rounded-t-xl flex justify-center items-center bg-grayBg h-10 border border-solid ${
							lock === 4 ? '' : 'border-r-0'
						}`}
					>
						<Lock />
			...
			<div className="bg-mainBeige grow">
				{owner.length ? (
					<></>
				) : (
