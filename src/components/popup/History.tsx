...
const UnLock = () => (
	...
	>
		...
		<defs>
			<pattern id="pattern0" patternContentUnits="objectBoundingBox" width="1" height="1">
				<use xlinkHref="#image0_538_1601" transform="scale(0.00195312)" />
			</pattern>
			<image
				id="image0_538_1601"
				width="512"
...
const BigLock = () => (
	...
	>
		...
		<defs>
			<pattern id="pattern0" patternContentUnits="objectBoundingBox" width="1" height="1">
				<use xlinkHref="#image0_577_1636" transform="scale(0.00195312)" />
			</pattern>
			<image
				id="image0_577_1636"
				width="512"
...
const History = () => {
	const { userId, userNickname } = store((state) => ({ userId: state.userId, userNickname: state.user?.nickname }))
	const [selectedHistory, setSelectedHistory] = useState<SelectedHistory | null>(null)
	const {
		data: { data: owner = [] } = {},
		refetch,
		isLoading
	} = useQuery(['room', 'user', 'owner', userId], () => UserApi.getOwnerDetail(userId || ''), {
		cacheTime: Infinity,
		staleTime: 1000 * 60,
		retry: false,
		...
		<div className="grow flex flex-col rounded-b-2xl w-full">
			<nav className="w-full grid grid-cols-5">
				{locks.map((lock, index) => (
					...
					>
						{index >= openedLockIndex ? <Lock /> : <UnLock />}
					</div>
				))}
			</nav>
			<div className="bg-mainBeige grow py-4 px-5 max-h-half overflow-y-scroll">
				{selectedHistory ? (
					<h1 className="text-mainTeal text-lg text-center mb-4">
						{userNickname}의 {numberToKr[selectedHistory.selectedIndex]} 파이
					</h1>
				) : null}
				{owner.length && selectedHistory ? (
					<div className="flex flex-col space-y-6">
						<div className="flex w-full justify-center text-mainTeal text-sm border-solid border-mainTeal rounded-xl border shadow-btnShadow py-1">
							얻은 페브
						</div>
						<div className="flex justify-between gap-x-2 border border-dashed rounded-lg p-4">
							<div className="flex-1">
								<img className="mx-auto max-w-[70%]" src={`/image/feve/${selectedHistory.item.userPieFeve.feveId}.png`} />
							</div>
					...
					<div className="w-full  flex flex-col justify-center items-center h-[calc(var(--vh,1vh)_*_60)] space-y-6">
						<BigLock />
